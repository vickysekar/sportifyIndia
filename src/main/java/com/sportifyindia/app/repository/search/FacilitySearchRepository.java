...
interface FacilitySearchRepositoryInternal {
    ...
    Page<Facility> search(Query query);

    @Async
    void index(Facility entity);
...
class FacilitySearchRepositoryInternalImpl implements FacilitySearchRepositoryInternal {
    ...
    FacilitySearchRepositoryInternalImpl(ElasticsearchTemplate elasticsearchTemplate, FacilityRepository repository) {
    ...
    @Override
    public Page<Facility> search(Query query) {
        SearchHits<Facility> searchHits = elasticsearchTemplate.search(query, Facility.class);
        List<Facility> hits = searchHits.map(SearchHit::getContent).stream().toList();
        return new PageImpl<>(hits, query.getPageable(), searchHits.getTotalHits());
    }

    @Override
    public void index(Facility entity) {
        repository.findById(entity.getId()).ifPresent(elasticsearchTemplate::save);
    }

    @Override
    public void deleteFromIndexById(Long id) {
        elasticsearchTemplate.delete(String.valueOf(id), Facility.class);
    }
}
